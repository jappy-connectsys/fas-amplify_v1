...
import { selectCompanies, selectCompanyId, ClearCompanies, UpdateCompany, GetCompanies } from './../../../../store/reducers/companySlice';
import { selectCurrencies, ClearCurrencies, GetCurrencies } from '../../../../store/reducers/currencySlice';
import { selectCountries, ClearCountries, GetCountries } from '../../../../store/reducers/countrySlice';
import { selectPaymentTerms, ClearPaymentTerms, GetPaymentTerms } from '../../../../store/reducers/paymenttermSlice';
import { selectPostingGroups, ClearPostingGroups, GetPostingGroups } from '../../../../store/reducers/pstgroupSlice';
import { selectPaymentModes, ClearPaymentModes, GetPaymentModes } from '../../../../store/reducers/paymentmodeSlice';
...
const CompanyUpdate = () => {
   ...
   useEffect(()=>{
      ...
      dispatch(ClearPaymentTerms());
      dispatch(ClearPostingGroups());
      dispatch(ClearPaymentModes());
   ...
   const { loading, message } = useSelector((state) => state.company);
   const data = useSelector((state) => selectCompanyId(state, Number(id)));
   
   /* Load Selection Options */
   const { data:pstgroupData } = useSelector((state) => state.pstgroup);
   const { data:currenciesData } = useSelector((state) => state.currency);
   const { data:countriesData } = useSelector((state) => state.country);
   const { data:ptermsData } = useSelector((state) => state.paymentterm);
   const { data:pmodesData } = useSelector((state) => state.paymentmode);

   console.log('data: ' + data + ' references: ' + ptermsData);
   ...
   const handleSubmit = () => {
      if(company_name === ""){
            setMes(<span style={{color:'red',fontWeight:600}}>Company Name is required.</span>)
      ...
      //      setMes(<span style={{color:'red',fontWeight:600}}>The Customer Name is required</span>)    
            ...
            if(type === "add"){
               dispatch(AddCompany(payload)).then(()=>{
                  renderSuccess(type, company_name)
               })
            }else{
   ...
   const renderSuccess = (type, companyName) => {
      if(type === "add"){
          ...
          setMes(<span style={{color:'green',fontWeight:600}}>
          {company_name}  has been successfully updated.
          </span>)   
      }
   ...
   const handleUpdate = (item) => {
      ...
      setCompanyShortName(item.company_short_name)
      setAddress1(item.address1)
      setAddress2(item.address2)
      setCity(item.city)
      setProvince(item.province)
      setPostCode(item.post_code)
      setCountry(item.country_abbr)
      setWebsite(item.website)
      setTin(item.tax_identification_number)
      setCurrencyCode(item.currency_code)
      setBusinessPostingGroup(item.business_posting_group)
      setVatPostingGroup(item.vat_posting_group)
      setCustomerPostingGroup(item.customer_posting_group)
      setContactFirstName(item.contact_person_first_name)
      setContactLastName(item.contact_person_last_name)
      ...
      setDateUpdated(item.date_updated)
